...
import 'package:themallbd_new/controllers/blogs/blog_view_controller.dart';
...
//this is when the app is in recent/background mode
  ...
  void handleDynamicLink(PendingDynamicLinkData dynamicLink) async {
    if (kDebugMode) {
      print(dynamicLink);
    }
    var pathSegments = dynamicLink.link.pathSegments;
    if (kDebugMode) {
      print(pathSegments);
    }
    ...
    else if (pathSegments.first == "product") {
      var param = dynamicLink.link.queryParameters;

      if (param["type"] == "life-style") {
    ...
    else if (pathSegments.first == "blog") {
      Get.toNamed("/blog_view",
          arguments: pathSegments.last
      );
    }
  }
  ...
  void handleDeepLink(Uri deepLink) {
    ...
    if (deepLink.pathSegments.isNotEmpty) {
      var pathSegments = deepLink.pathSegments;
      ...
      switch (pathSegments.first) {
        case 'sale':
          ...
          break;
        case 'product':
          Get.toNamed('/product_details_page', arguments: [pathSegments.last]);
          break;
        case 'blog':
          Get.put(BlogViewController()).key.value=pathSegments.last;
          Get.find<BlogViewController>().fetchData();
          Get.toNamed('/blog_view', arguments: pathSegments.last);
          break;
        default:
          if (kDebugMode) {
            print('Unknown path: ${deepLink.path}');
          }
      }
    }
  }
...
/*  void fetchData() async{
    var data=await RemoteServices.fetchHomePageData(url.toString()+endPoint.toString());
    if(data!=null){
      homePageDataModel.value=data;
      homePageSkinTypeList.value=data.skinTypes!;
      homePageCategoryList.value=data.category!;
                  ...
                  MaterialButton(
                    ...
                    onPressed: () {
                      ...
                      /*if (Platform.isIOS) {
                          ...
                          final url = Uri.parse(appVersionData
                              .value.majorMsg!.url!.ios
                              .toString());
                          launchUrl(
                            url,
                            mode: LaunchMode.externalApplication,
